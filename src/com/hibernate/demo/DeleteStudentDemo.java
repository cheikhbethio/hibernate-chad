...
public class DeleteStudentDemo {
	...
	public static void main(String[] args) {
		...
		try {			
			...
			System.out.println("deleting student1 first way........");
			session = factory.getCurrentSession();
			session.beginTransaction();
			Student gettedStudent= session.get(Student.class, createdStudent1.getId());
			session.delete(gettedStudent);
			session.getTransaction().commit();
			System.out.println("deleted student" + gettedStudent);
			

			System.out.println("deleting all students second way........");
			session = factory.getCurrentSession();
			session.beginTransaction();
